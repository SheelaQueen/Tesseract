...
class TesseractDataResolver {
    ...
    resolveChannel(channel, channels) {
        ...
        if (channel instanceof discord_js_1.Channel)
            return channel;
        if (typeof channel === "string")
            return channels.get(channel)
                ...
                || null;
        if (channel instanceof discord_js_1.Message)
            return channel.channel;
        if (channel instanceof discord_js_1.Guild)
    ...
    resolveUser(user) {
        ...
        if (user instanceof discord_js_1.GuildMember)
            return user.user;
        if (user instanceof discord_js_1.Message)
            return user.author;
        if (user instanceof discord_js_1.Guild)
